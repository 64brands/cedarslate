...
export async function onRequestPost({request,env}){
  try{
    ...
    const name=clean(body.name,80),email=clean(body.email,160),phone=clean(body.phone,30),message=clean(body.message,3000);
    if(name.length<2||message.length<10||email.length<5||!email.includes("@"))return json({error:"Please complete every field."},400);
    const safeName=escapeHtml(name),safeEmail=escapeHtml(email),safePhone=escapeHtml(phone||"Not provided"),safeMessage=escapeHtml(message).replace(/\n/g,"<br>");
    const sent=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer `+env.RESEND_API_KEY,"Content-Type":"application/json"},body:JSON.stringify({from:"Cedar+Slate Website <website@cedarslate.com.au>",to:["hello@cedarslate.com.au"],reply_to:email,subject:`Website enquiry from `+name,text:`Name: `+name+`\nEmail: `+email+`\nPhone: `+(phone||"Not provided")+`\n\n`+message,html:`<p><strong>Name:</strong> `+safeName+`</p><p><strong>Email:</strong> `+safeEmail+`</p><p><strong>Phone:</strong> `+safePhone+`</p><p>`+safeMessage+`</p>`})});
